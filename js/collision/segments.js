gA.segment = (function() {
  ...
  var Point = function(x, y) {
    ...
    this.draw = function(size, color) {
      gA.ctx.g.beginPath();
      gA.ctx.g.arc(this.x, this.y, size, 0, 2*Math.PI);
      gA.ctx.g.closePath();
      gA.ctx.g.fillStyle = color;
      gA.ctx.g.fill();
    };
  };

  function make2(x, y, vecx, vecy) {
    this.x = x;
    this.y = y;
    this.vecx = vecx;
    this.vecy = vecy;
    ...
    this.intersect = function(segment) {
      ...
      num = (b1 * c2) - (b2 * c1);
      if (num < 0 ) intersectX = (num - offset) / denom; else intersectX = (num + offset) / denom;

      num = (a2 * c1) - (a1 * c2);
      if (num < 0) intersectY = (num - offset ) / denom; else intersectY = (num + offset) / denom;

      // new Point(intersectX, intersectY).draw(3, 'blue'); //Drawing a point
